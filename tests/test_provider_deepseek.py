...
- ``ENDPOINT`` resolves through profile-driven base URL,
- ``base_url`` profile override redirects the request,
- factory wiring resolves ``deepseek`` to ``DeepSeekProvider``.
"""
...
def test_factory_resolves_deepseek() -> None:
...
def test_deepseek_endpoint_override() -> None:
    with patch(
        ...
        provider = DeepSeekProvider()
        assert provider.ENDPOINT == "https://proxy.example.com/v1/chat/completions"


def test_deepseek_call_returns_tool_calls() -> None:
    provider = DeepSeekProvider()
    raw_tc = [{
        "id": "call_xyz",
        "type": "function",
...
def test_deepseek_text_only_response_stops() -> None:
    provider = DeepSeekProvider()
    fake_resp = _mock_chat_completion(content="Done.", tool_calls=None)
...
def test_deepseek_endpoint_override_routes_request() -> None:
    ...
    call_args = mock_client.post.call_args
    assert call_args[0][0] == "https://proxy.example.com/v1/chat/completions"


def test_deepseek_append_tool_results_uses_tool_role() -> None:
    """Inherited from OpenAIProvider: each result is a separate ``role: tool`` msg."""
    provider = DeepSeekProvider()
    messages: list[dict] = []
    provider.append_tool_results(messages, [("call_a", "RESULT_A"), ("call_b", "RESULT_B")])

    assert len(messages) == 2
    assert messages[0] == {"role": "tool", "tool_call_id": "call_a", "content": "RESULT_A"}
    assert messages[1] == {"role": "tool", "tool_call_id": "call_b", "content": "RESULT_B"}
