...
from axiom.db import format_prefixed_id, get_db, init_db, kv_get, kv_set, log_activity, next_container_id
...
from axiom.market_cache import (
    load_price_snapshot,
...
from axiom.trade_state import close_trade_record, mark_trade_pending_close_reconcile, parse_trade_signal_data

log = logging.getLogger("axiom.scanner")
...
def _get_account_equity() -> float:
    ...
    from axiom.sim.clock import is_sim_active, sim_kv_key
...
def _load_live_price_cache() -> tuple[dict[str, float], float | None]:
    ...
    try:
        ...
        pass
    try:
        from axiom.exchange.hyperliquid import get_all_mids
        prices = get_all_mids()
        if prices:
            return prices, 0.0
...
def _queue_trade_execution_intent(intent: dict) -> tuple[bool, str | None, str | None]:
    ...
    try:
        from axiom.brain import assign_task_direct
...
def _paper_test_mode_enabled() -> bool:
    ...
    try:
        from axiom.api_domains.paper import _apply_paper_test_settings
...
def fetch_candles(coin: str, bars: int = 300, interval: str = "1h") -> pd.DataFrame:
    ...
    from axiom.sim.clock import is_sim_active, get_now
    if is_sim_active():
...
def _enrich_scan_frame(df: pd.DataFrame, asset: str, timeframe: str) -> pd.DataFrame:
    ...
    if df is None or df.empty:
        return df
    try:
        from axiom.strategies.backtest import _enrich_with_market_data
...
def get_signal(
    ...
    if strategy_instance is None:
        try:
            from axiom.strategies.registry import _TYPE_MAP, get_active, resolve_runtime_type
...
def _report_execution_failure(strategy_id: str | None, action: str, trade_id: str | None, reason: str | None = None) -> None:
    ...
    try:
        from axiom.brain import handoff_execution_failure_to_developer
...
def _execute_direct(
    ...
    if action == "close":
        ...
        try:
            from axiom.exchange.hyperliquid import get_positions as _get_positions_for_funding
...
def _resolve_trade_vault_address(trade_id, *, strict: bool = False) -> str | None:
    ...
    try:
        from axiom.exchange import books
        from axiom.db import kv_get as _kv_get
        with get_db() as conn:
        ...
        settings = _kv_get("axiom:settings", {})
        if not isinstance(settings, dict):
            settings = {}
    ...
    for strat_id, strat in strategy_items:
        try:
            ...
            if _last_ts is not None:
                ...
                try:
                    _max_bars = float((kv_get("axiom:settings", {}) or {}).get("scanner_max_candle_staleness_bars", 2) or 2)
                except Exception:
                    _max_bars = 2.0
        ...
        for future in as_completed(futures):
            ...
            for item in scanned:
                try:
                    ...
                    try:
                        from axiom.db import record_signal_result
...
def run_scan(*, execute_positions: bool = True) -> dict:
    ...
    try:
        from axiom.strategies.registry import discover, get_active
        discover()
    ...
    asset_regimes: dict[str, object] = {}
    try:
        from axiom.regime import detect_regime
...
def sweep_pending_close_reconcile() -> dict:
    ...
    """
    from axiom.db import get_db, log_activity, kv_set
    ...
    for row in trades:
        ...
        if outcome == "closed_locally_paper_local":
            ...
            try:
                from axiom.exchange.hyperliquid import close_position
...
def record_scan_slo(
    ...
    from axiom.db import kv_set, log_activity
