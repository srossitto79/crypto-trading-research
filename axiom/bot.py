...
This is the equivalent of OpenClaw's gateway. When the operator sends a message:
1. Bot receives it
...
from axiom.config import AXIOM_HOME, ensure_dirs, load_config
...
from axiom.db import create_pending_task, kv_get, kv_set, kv_set_best_effort
from axiom.model_routing import get_default_model_for_provider
from axiom.notification_policy import DEFAULT_RESPONSE_CHANNEL_ALIASES
from axiom.notification_renderers import summarize_discord_text
from axiom.ai import _is_rate_limit_exception, is_transient_provider_exception, normalize_provider_and_model
...
def _bot_owns_runtime_loops() -> bool:
    ...
    """
    return os.environ.get("AXIOM_BOT_OWNS_RUNTIME", "").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
...
_DEFAULT_CHANNELS = {
    "general": "1519376562938581003",
    "ops": "1519641298343034940",
    "approvals": "1519641373630795787",
    "risk": "1519641373630795787",
    "morning-brief": "1519641071984840814",
    "evening-brief": "1519641172887212182",
    "evening-summary": "1519641172887212182",
    "chat": "1519641215895605278",
    "heartbeat": "1519641257285124146",
    "development": "1519641298343034940",
    "strategies": "1519641333604810782",
    "alerts": "1519641373630795787",
    "research": "1519641416043859989",
    "backtesting": "1519641449392767146",
    "paper-trades": "1519641517608800307",
    "market-data": "1519641564719218760",
    "autopilot": "1519641605114560623",
    "news": "1519641638840832120",
    "full-stack-engineer": "1519641688224567427",
    "quant-researcher": "1519641416043859989",
    "back-test-engineer": "1519641449392767146",
    "risk-manager": "1519641373630795787",
    "sentiment": "1519641638840832120",
    "full-stack-engineers": "1519641298343034940"
}
...
def _load_respond_channels() -> set[str]:
    ...
    preferences = kv_get("axiom:notification_preferences", {}) or {}
...
def _render_operational_discord_reply(response: str, *, source: str = "", task_message: str = "") -> str:
    ...
    compact = summarize_discord_text(response, limit=420, max_lines=3) or _single_line(response) or "axiom update."
...
def get_bot_token() -> str:
    ...
    if not token:
        raise ValueError(
            ...
            "or add 'discord_token' to ~/.Axiom/config.json"
        )
    return token
    ...
    return [
        {
            ...
            "visibility": "visible",
            "instructions": (
                "You are the Quant Researcher for Axiom. Your focus is benchmarking, market structure research, data-gap discovery, and data integrity.\n"
        ...
        },
        {
            ...
            "visibility": "visible",
            "instructions": (
                "You are the Simulation Agent for Axiom. Your focus is Strategy Container Validation.\n"
        ...
        },
        {
            ...
            "visibility": "visible",
            "instructions": (
                "You are the Risk Manager for Axiom. Your focus is Live Strategy Container Oversight and Capital Allocation.\n"
        ...
        },
        {
            ...
            "visibility": "visible",
            "instructions": (
                "You are the Execution Trader for Axiom. You are the ONLY agent with exchange access.\n"
        ...
        },
        {
            ...
            "visibility": "visible",
            "instructions": (
                "You are the Strategy Developer for Axiom. You own the full hypothesis-to-strategy loop.\n"
        ...
        },
        {
            ...
            "visibility": "visible",
            "instructions": (
                "You are the Full-Stack Engineer for Axiom — the operator-facing triage and diagnosis agent.\n"
        ...
        },
        {
            "agent_id": "brain",
            "name": "Brain",
            "role": "Central orchestrator and decision layer for Axiom. Delegates work and arbitrates between agents.",
            "model": "openai",
            "model_id": get_default_model_for_provider("openai"),
            "visibility": "visible",
            "instructions": (
                "You are the Brain for Axiom. Your focus is Strategy Container Lifecycle Management.\n"
...
class AxiomBot(commands.Bot):
    """Axiom Discord bot — gateway to the AI brain."""
    ...
    async def setup_hook(self):
        ...
        if not self.agent_id and _bot_owns_runtime_loops():
            ...
            log.info(
                ...
                "(set AXIOM_BOT_OWNS_RUNTIME=1 to override)."
            )
    ...
    async def on_ready(self):
        ...
        from axiom.scheduler import reset_scheduler_job_locks
    ...
    async def _bootstrap(self):
            ...
            if should_queue_bootstrap_brain_cycle():
                create_pending_task(
                    ...
                    "brain_invoke",
                    {
                        "source": "bootstrap",
                        "message": (
                            "axiom just started. You are the Brain — the sole orchestrator.\n\n"
        ...
        boot_msg = f"axiom BOOTSTRAP COMPLETE\n{summary}"
        log.info(boot_msg)
    ...
    async def on_message(self, message: discord.Message):
        ...
        if self.agent_id:
            ...
            log.info("Agent message from operator to %s in #%s: %s", self.agent_id, channel_name, content[:100])
            ...
            async with message.channel.typing():
                try:
                    ...
                    log_activity(
                        ...
                        f"[#{channel_name}] Operator: {content[:100]} → Response: {response[:100]}",
                    )
                except Exception as e:
        ...
        # Only process messages from the operator in allowed channels/DMs
        ...
        log.info("Message from operator in #%s: %s", channel_name, content[:100])
        ...
        async with message.channel.typing():
            try:
                ...
                # Log to activity
                from axiom.db import log_activity
                log_activity("info", "bot", f"[#{channel_name}] Operator: {content[:100]} → Response: {response[:100]}")
    ...
    async def _generate_agent_response(self, content: str, channel_id: str, channel_name: str) -> str:
        ...
        preamble = (
            f"You are {agent_name}, a specialist agent in the Axiom team.\n"
            f"Your role: {agent_role or 'Specialist operator.'}\n"
            "Stay in this persona and never answer as Axiom (the Brain)."
        )
        context = preamble + "\n\n---\n\n" + context
        ...
        if history:
            ...
            for msg in history[-10:]:
                role = "Operator" if msg["role"] == "user" else agent_name
        ...
        context += (
            "\n\n---\n\n# CHAT MODE\n"
            f"You are in CHAT mode responding to the operator as {agent_name} on Discord.\n"
            ...
            f"If an action request reaches you without being queued, tell the operator to use `!assign {agent_id}: <task>` on the main Axiom bot.\n"
        ...
        try:
            append_workspace(
                ...
                f"**Operator**: {content[:200]}\n**{agent_name}**: {response[:200]}\n",
            )
        except Exception as e:
            log.warning("Agent memory append failed (%s): %s", agent_id, e)

        try:
            summary = f"[Discord agent:{agent_id} #{channel_name}] Operator: {content[:200]} | {agent_name}: {response[:300]}"
            store_narrative(
                summary,
                metadata={
                    "type": "conversation",
    ...
    async def _generate_response(self, content: str, channel_id: str, channel_name: str, is_dm: bool) -> str:
        ...
        if history:
            context += "\n\n---\n\n# RECENT CONVERSATION IN THIS CHANNEL\n"
            for msg in history[-10:]:
                role = "Operator" if msg["role"] == "user" else "Axiom"
                context += f"\n**{role}**: {msg['content'][:300]}\n"
        ...
        context += (
            "\n\n---\n\n# CHAT MODE\n"
            "You are in CHAT mode responding to the operator on Discord.\n"
            "You do NOT have tools, function calls, or API access. Do NOT generate XML, "
            "tool_call blocks, invoke tags, or any structured function calls.\n"
            ...
            "If an action request reaches you without being queued, tell the operator to use `!brain <task>` or `!assign <agent> <task>`."
        )
        ...
        from axiom.workspace import append_workspace, today_memory_path
        try:
            append_workspace(
                today_memory_path(),
                f"\n### Discord #{channel_name} — {datetime.now(timezone.utc).strftime('%H:%M UTC')}\n"
    ...
    async def _process_pending_tasks(self):
        ...
        from axiom.db import get_db, claim_pending_tasks
        ...
        if not rows:
            ...
            try:
                from axiom.runtime_worker import ensure_brain_keepalive
        ...
        for task in rows:
            ...
            try:
                ...
                if source == "bootstrap":
                    from axiom.brain import assign_research_cycle
                ...
                if is_chat:
                    ...
                    from axiom.context import build_chat_context
                    context = build_chat_context()
                    ...
                    _chat_tool_names = {
                        ...
                        # Backtesting tools
                        "AXIOM_run_backtest", "AXIOM_list_datasets", "AXIOM_get_results",
                    }
                    ...
                    if chat_tools:
                        context += (
                            ...
                            "- **AXIOM_run_backtest**: Run a backtest via the Axiom pipeline\n"
                            "- **AXIOM_list_datasets**: List available datasets\n"
                            "- **AXIOM_get_results**: Retrieve backtest results\n\n"
                    ...
                    context += (
                        ...
                        "## Backtesting (Axiom)\n"
                        "- **AXIOM_list_datasets**: List available backtesting datasets\n"
                        "- **AXIOM_create_strategy**: Create a strategy on Axiom Backtesting\n"
                        "- **AXIOM_run_backtest**: Run a backtest with realistic fees\n"
                        "- **AXIOM_run_optimization**: Run parameter optimization\n"
                        "- **AXIOM_run_verdict**: Validate a strategy\n"
                        "- **AXIOM_get_results**: Get detailed backtest results\n\n"
                    ...
                    try:
                        ...
                        store_narrative(
                            ...
                            metadata={"type": "brain_cycle", "source": "Axiom"},
                        )
                    except Exception:
                        pass
                ...
                if isinstance(e, asyncio.TimeoutError):
                    from axiom.db import requeue_brain_task
...
async def send(channel_name: str, message: str, channel_id: str | None = None, agent_id: str | None = None):
    ...
    async def _send_with_bot(selected_bot: AxiomBot):
...
async def send_thread(channel_name: str, title: str, message: str, channel_id: str | None = None, embeds: list[dict] | None = None, agent_id: str | None = None):
    ...
    async def _send_thread_with_bot(selected_bot: AxiomBot):
        await selected_bot.wait_until_ready_custom()
        channel = selected_bot.get_channel(int(target_id))
        if not channel:
            channel = await selected_bot.fetch_channel(int(target_id))
        ...
        try:
            with httpx.Client(timeout=request_timeout_seconds) as client:
                ...
                if view_resp.status_code == 200:
                    ...
                    if probe_send:
                        probe_msg = (
                            f"[Axiom Discord audit] {actor} probe at "
...
def run_bot():
    ...
    if not _acquire_bot_lock():
        ...
        log.warning("Another bot instance is already running%s; skipping duplicate start.", pid_suffix)
        try:
            from axiom.db import init_db, log_activity
