...
log = logging.getLogger("axiom.security")
...
def _auth_required() -> bool:
    return _env_truthy("AXIOM_AUTH_REQUIRED")
...
def resolved_bind_host() -> str:
    ...
    ``AXIOM_HOST`` as a ``--host`` fallback, but the fail-closed guard used to
    read only ``AXIOM_BIND_HOST`` — so ``AXIOM_HOST=0.0.0.0`` with no API key
    ...
    actual bind.
    """
    for name in ("AXIOM_BIND_HOST", "AXIOM_HOST"):
...
def assert_safe_bind_host(bind_host: str) -> None:
    ...
    we fail closed there: an exposed bind must come with AXIOM_API_KEY (or an
    explicit AXIOM_AUTH_REQUIRED=true).
    """
    if is_loopback_host(bind_host):
        return
    if _read_env_secret("AXIOM_API_KEY") or _auth_required():
        return
    raise RuntimeError(
        f"Refusing to start: the API is bound to {bind_host!r} (reachable beyond "
        "localhost) but AXIOM_API_KEY is not set. Set AXIOM_API_KEY (and "
        "AXIOM_OPERATOR_KEY) in your .env, or bind to 127.0.0.1."
    )


def assert_auth_keys_configured() -> None:
    """Call at app startup. Refuse to start if auth is required but unset."""
    api_key = _read_env_secret("AXIOM_API_KEY")
    operator_key = _read_env_secret("AXIOM_OPERATOR_KEY")
    required = _auth_required()
    if required and not api_key:
        raise RuntimeError(
            "AXIOM_AUTH_REQUIRED=true but AXIOM_API_KEY is unset. "
            "Set AXIOM_API_KEY in .env or unset AXIOM_AUTH_REQUIRED for dev."
        )
    if required and not operator_key:
        raise RuntimeError(
            "AXIOM_AUTH_REQUIRED=true but AXIOM_OPERATOR_KEY is unset. "
            "Set AXIOM_OPERATOR_KEY in .env or unset AXIOM_AUTH_REQUIRED for dev."
        )
    if not api_key:
        log.warning(
            "AXIOM_API_KEY is unset — API is unauthenticated. "
            "Set AXIOM_AUTH_REQUIRED=true to refuse start without keys."
        )
    if not operator_key:
        log.warning(
            "AXIOM_OPERATOR_KEY is unset — operator endpoints are unauthenticated. "
            "Set AXIOM_AUTH_REQUIRED=true to refuse start without keys."
        )
    # Fail closed if the API is exposed beyond loopback without a key. Use the
    # launcher-aware resolver (AXIOM_BIND_HOST → AXIOM_HOST → 127.0.0.1) so the
    # AXIOM_HOST alias cannot smuggle a public bind past the guard (M6).
    assert_safe_bind_host(resolved_bind_host())


def require_api_access(request: HTTPConnection) -> None:
    expected_api_key = _read_env_secret("AXIOM_API_KEY")
...
def require_operator_access(request: HTTPConnection) -> None:
    ...
    expected_operator_key = _read_env_secret("AXIOM_OPERATOR_KEY")
    ...
    raise HTTPException(
        status_code=401,
        detail="Invalid or missing operator key. Run 'python -m axiom auth init-operator-key' to generate one, then add it to your .env file."
    )
...
async def require_api_access_ws(ws: HTTPConnection) -> bool:
    ...
    when no AXIOM_API_KEY is set (the default localhost setup), but when a key
    ...
    """
    expected = _read_env_secret("AXIOM_API_KEY")
    if not expected:
        return True
...
def get_allowed_cors_origins() -> list[str]:
    configured = _normalize_secret(os.environ.get("AXIOM_CORS_ORIGINS"))
    if configured:
        values = configured.split(",")
    else:
        frontend_port = _normalize_secret(os.environ.get("FRONTEND_PORT")) or "5173"
        api_port = _normalize_secret(os.environ.get("AXIOM_PORT")) or "8003"
        values = [
            f"http://127.0.0.1:{frontend_port}",
            f"http://localhost:{frontend_port}",
            f"http://127.0.0.1:{api_port}",
            f"http://localhost:{api_port}",
            _normalize_secret(os.environ.get("AXIOM_CLIENT_BASE")),
        ]
