"""Token storage, auto-refresh, and migration for Axiom auth."""
...
from axiom.config import AUTH_FILE, OPENCLAW_AUTH, ensure_dirs
from axiom.secret_storage import decrypt_secret, encrypt_secret

console = Console()
log = logging.getLogger("axiom.auth.store")
...
def _refresh_openai(profile: dict) -> dict:
    ...
    from axiom.auth.openai import CLIENT_ID, TOKEN_URL
...
def _refresh_minimax(profile: dict) -> dict:
    ...
    from axiom.auth.minimax import CLIENT_ID, TOKEN_URL
...
def get_token(provider: str) -> str:
    ...
    if _is_expired(profile):
        ...
        if refresher and profile.get("refresh"):
            ...
            raise RuntimeError(f"{provider} token expired and no refresh available. Run: Axiom auth login {provider}")
    ...
    def __init__(self, provider: str, status: str = "missing"):
        ...
        if status == "opaque":
            ...
            msg = (
                ...
                f"(or run: Axiom auth login {self.provider})."
            )
        else:  # missing / unknown
            msg = (
                f"{self.provider} has no API credentials configured. Add them in "
                f"Settings > Agents > AI providers (or run: Axiom auth login {self.provider})."
            )
        super().__init__(msg)
...
def display_status():
    ...
    table = Table(title="axiom Auth Status")
...
def migrate_from_openclaw():
    ...
    # Map OpenClaw provider names to Axiom names
    provider_map = {
    ...
    for oc_key, oc_profile in oc_profiles.items():
        ...
        AXIOM_provider = provider_map.get(oc_provider)
        if not AXIOM_provider:
            console.print(f"  [dim]Skipping unknown provider: {oc_provider}[/dim]")
            continue

        AXIOM_key = f"{AXIOM_provider}:default"

        # Build normalized profile
        profile = {"type": oc_profile.get("type", "oauth"), "provider": AXIOM_provider}
        ...
        store["profiles"][AXIOM_key] = profile
        migrated += 1
        console.print(f"  [green]Migrated {AXIOM_provider}[/green]")
...
def run_login(provider: str):
    ...
    flows = {
        "openai": "axiom.auth.openai",
        "minimax": "axiom.auth.minimax",
    }
    import importlib
