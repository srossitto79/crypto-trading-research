...
from axiom.config import get_execution_mode, load_config
from axiom.circuit_breaker import hl_price_breaker, hl_trade_breaker, hl_account_breaker
from axiom.db import kv_get

log = logging.getLogger("axiom.exchange.hl")
...
# increasing nonces. Axiom signs everything with ONE master key (vault-routed
...
class HyperliquidInfoClient(Protocol):
    """Subset of HyperLiquid info methods used by Axiom."""
...
def _assert_execution_allowed(testnet: bool) -> None:
    ...
    ``AXIOM_ALLOW_MAINNET=1`` opt-in, otherwise we refuse. This collapses the
    ...
    if testnet:
        return
    if _is_truthy(os.environ.get("AXIOM_ALLOW_MAINNET")):
        return
    raise RuntimeError(
        "Refusing to place a MAINNET order: resolved testnet=False but "
        "AXIOM_ALLOW_MAINNET is not set. This is the stable-release safety guard — "
        "set AXIOM_ALLOW_MAINNET=1 only when you intentionally trade real funds."
    )


def _hl_encryption_disabled() -> bool:
    env_value = os.environ.get("AXIOM_HL_DISABLE_ENCRYPTION")
...
def _load_creds_from_AXIOM_settings() -> dict:
    try:
        settings = kv_get("axiom:settings", {}) or {}
        secrets = kv_get("axiom:settings:secrets", {}) or {}
    except Exception:
        return {}
...
def _load_creds_from_env() -> dict:
    ...
    env_map = {
        "HL_API_SECRET": _read("AXIOM_HL_API_SECRET") or _read("HL_API_SECRET"),
        "HL_API_KEY": _read("AXIOM_HL_API_KEY") or _read("HL_API_KEY"),
        "HL_WALLET_ADDRESS": _read("AXIOM_HL_WALLET_ADDRESS") or _read("HL_WALLET_ADDRESS"),
        "USE_TESTNET": _read("AXIOM_HL_USE_TESTNET") or _read("USE_TESTNET"),
    }
...
def _get_creds_path() -> str:
    ...
    path = os.environ.get("AXIOM_HL_CREDS_PATH")
    if not path:
        cfg = load_config()
...
def _get_creds() -> dict:
    ...
    settings_creds = _load_creds_from_AXIOM_settings()
    if settings_creds:
    ...
    if not settings_file.exists():
        raise FileNotFoundError(
            ...
            "or set AXIOM_HL_WALLET_ADDRESS/AXIOM_HL_API_SECRET env vars, "
    ...
    if not key_file.exists():
        ...
        raise FileNotFoundError(
            ...
            "(set AXIOM_HL_DISABLE_ENCRYPTION=1 for plaintext settings)"
        )
...
def _get_account_info_client(testnet: bool = True) -> tuple[HyperliquidInfoClient, str]:
    ...
    if not wallet:
        raise RuntimeError(
            ...
            "Set Settings > HyperLiquid wallet address or AXIOM_HL_WALLET_ADDRESS."
        )
    ...
    *,
    ...
    if not pk:
        raise RuntimeError(
            ...
            "Set Settings > HyperLiquid private key or AXIOM_HL_API_SECRET."
        )
...
def market_order(
    ...
    """
    from axiom.sim.clock import is_sim_active
    if is_sim_active():
        from axiom.sim.mock_exchange import sim_market_order
...
def close_position(
    ...
    from axiom.sim.clock import is_sim_active
    if is_sim_active():
        from axiom.sim.mock_exchange import sim_close_position
...
def get_positions(testnet: bool = True, *, account_address: str | None = None) -> dict:
    ...
    from axiom.sim.clock import is_sim_active
    if is_sim_active():
        from axiom.sim.mock_exchange import sim_get_positions
...
def get_account_value(
    ...
    from axiom.sim.clock import is_sim_active
    if is_sim_active():
        from axiom.sim.mock_exchange import sim_get_account_value
...
def get_all_mids(testnet: bool = True) -> dict[str, float]:
    ...
    from axiom.sim.clock import is_sim_active
    if is_sim_active():
        from axiom.sim.mock_exchange import sim_get_all_mids
...
def get_exchange(testnet: bool = True) -> "ExchangeInterface":
    ...
    if _default_exchange is None:
        from axiom.exchange.sync_wrapper import _build_exchange_for_settings
