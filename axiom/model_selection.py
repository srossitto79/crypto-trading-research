...
* CONNECTED — a provider is "connected" only if the operator explicitly added
  it in the app (persisted in the ``Axiom:connected-providers`` KV set) AND a
...
log = logging.getLogger("axiom.model_selection")

_CONNECTED_PROVIDERS_KEY = "axiom:connected-providers"
...
_ENFORCEMENT_KEY = "axiom:model-selection-enforced"
...
def migrate_connected_from_profiles() -> set[str]:
    ...
    """
    try:
        from axiom.auth.store import load_auth
    except Exception:
