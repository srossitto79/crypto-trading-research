...
from axiom.codex_responses import call_codex, is_openai_oauth_token, stream_codex

log = logging.getLogger("axiom.agents.providers")
...
class CodexProvider(OpenAIProvider):
    ...
    ChatGPT subscription OAuth tokens (minted by ``Axiom.auth.openai``) are
    ...
    ``Axiom.codex_responses`` converts to Responses ``input`` items. The raw
...
class OpenRouterProvider(OpenAIProvider):
    ...
    ``anthropic/claude-sonnet-4``). Axiom's ``provider:model`` routing
...
class AnthropicProvider(ToolCallProvider):
    ...
    Profile (``Axiom.auth.store.get_profile('anthropic')``) supports
