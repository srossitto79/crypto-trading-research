...
import httpx
from axiom.model_routing import get_model_routing_snapshot

from axiom.auth.store import get_profile, get_token
...
_DEFAULT_READ_TIMEOUT_SECONDS = float(
    os.environ.get("AXIOM_AI_READ_TIMEOUT_SECONDS", "120.0")
)
...
def _record_call_health(provider: str, error: Exception | None = None) -> None:
    ...
    try:
        from axiom.provider_runtime_health import record_call_failure, record_provider_ok
...
def _record_fallback_event(primary: str, used: str) -> None:
    ...
    try:
        from axiom.provider_runtime_health import record_provider_event
...
async def _call_openai(
    ...
    if provider_label == "openai" and is_openai_oauth_token(token):
        from axiom.codex_responses import call_codex
