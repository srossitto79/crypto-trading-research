...
_REMOTE_DATA_ROOT_ENV = "AXIOM_REMOTE_ENGINE_DATA_ROOT"
_REMOTE_DATA_ALLOWED_ROOT_ENV = "AXIOM_REMOTE_ENGINE_ALLOWED_ROOT"
...
def _normalize_dataset_rows(raw_rows: list[dict]) -> list[dict[str, object]]:
    from axiom.data import classify_dataset_asset_class, dataset_market_type
...
def _scan_remote_data_root_datasets(remote_root: str) -> list[dict[str, object]]:
    from axiom.data import _dataset_from_file
...
def get_data_ingestion_runs(
    ...
    from axiom.data import get_active_ingestion_runs, scan_datasets
...
def get_cached_data_ingestion_runs(
    ...
    from axiom.data import get_active_ingestion_runs, peek_cached_datasets
...
def post_data_ingestion_submit(
    ...
    if remote_enabled and not remote_skip:
        ...
        try:
            ...
            from axiom.data import _ingestion_runs, _ingestion_runs_lock
    ...
    from axiom.data import submit_ingestion
...
def get_cached_datasets_stub() -> list[dict[str, object]]:
    try:
        from axiom.data import peek_cached_datasets
...
def get_dataset_detail_stub(symbol: str, timeframe: str, remote_skip: bool = False):
    ...
    from axiom.data import get_dataset_detail
...
def delete_dataset_stub(symbol: str, timeframe: str, remote_skip: bool = False):
    ...
    from axiom.data import delete_dataset
...
def get_data_quality(symbol: str, timeframe: str, remote_skip: bool = False):
    ...
    from axiom.data import compute_data_quality
...
def get_data_health():
    ...
    :func:`Axiom.data.compute_data_health` is preserved so the frontend
    ...
    """
    from axiom.data import compute_data_health
    from axiom.data_manager import _now_iso, data_manager_stats
...
def _probe_lan_health() -> list[dict]:
    ...
    from axiom.lan_enricher import _MAX_STALENESS_MULT, _SKIP_COLS
...
def get_collection_health() -> dict:
    ...
    from axiom.health_monitor import DATA_STREAM_FAILURE_RED, data_health_score
...
def get_data_activity(limit: int = 200) -> dict:
    ...
    try:
        from axiom.data import get_active_ingestion_runs
...
def post_scan_orphans() -> dict:
    ...
    from axiom.data import scan_parquet_orphans
...
def post_cleanup_orphans() -> dict:
    ...
    from axiom.data import cleanup_parquet_orphans
...
def execute_data_engine_catchup(
    ...
    endpoint and the scheduled ``Axiom-data-engine-catchup`` auto-drain job can
    ...
    from axiom.data import backfill_ohlcv_gaps
...
def post_upload_csv_preview(content: bytes):
    from axiom.data import preview_csv
...
def post_upload_csv(
    ...
    from axiom.data import process_csv_upload
...
def get_dataset_export(symbol: str, timeframe: str, format: str = "csv") -> tuple[bytes, str, str]:
    from axiom.data import export_dataset_bytes
...
def get_symbol_export(symbol: str, format: str = "csv") -> tuple[bytes, str]:
    from axiom.data import export_symbol_zip
...
def _build_ohlcv_response(symbol: str, timeframe: str = "1h", limit: int = 100) -> dict:
    ...
    from axiom.api_domains.trading import _coerce_iso_timestamp, _normalize_asset_key
...
def get_sources_stub():
    try:
        from axiom.data import list_data_sources
...
def get_source_symbols_stub(source: str, query: str | None = None, exchange: str | None = None):
    try:
        from axiom.data import search_source_symbols
...
def get_stream_health(symbol: str) -> dict:
    ...
    try:
        from axiom.data import load_parquet, symbol_to_fs
...
def get_stream_rows(
    ...
    try:
        ...
        from axiom.data_manager import FUNDING_DIR, OI_DIR, _load_stream_parquet, data_manager
...
def get_coverage() -> dict:
    ...
    from axiom.data_manager import FUNDING_DIR, OI_DIR
