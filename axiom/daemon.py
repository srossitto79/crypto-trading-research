"""axiom Trading Daemon — data ingestion and hard-risk loop.
...
from axiom.config import AXIOM_HOME, ensure_dirs, get_execution_mode
from axiom.db import get_db, get_open_trades, init_db, kv_get, kv_set_best_effort, log_activity
...
from axiom.market_cache import normalize_prices, publish_price_snapshot, publish_candle_snapshot
from axiom.market_data import fetch_hyperliquid_candles, dataframe_to_ohlcv_rows
from axiom.runtime_health import compute_runtime_code_fingerprint
...
PRICE_SNAPSHOT_TIMEOUT_SECONDS = _env_float("AXIOM_DAEMON_PRICE_SNAPSHOT_TIMEOUT_SECONDS", 6)
RISK_ACCOUNT_TIMEOUT_SECONDS = _env_float("AXIOM_DAEMON_ACCOUNT_TIMEOUT_SECONDS", 10)
RISK_UPDATE_TIMEOUT_SECONDS = _env_float("AXIOM_DAEMON_RISK_UPDATE_TIMEOUT_SECONDS", 8)
RISK_RECONCILE_TIMEOUT_SECONDS = _env_float("AXIOM_DAEMON_RECONCILE_TIMEOUT_SECONDS", 20)
...
_RECONCILE_ERROR_ESCALATE_AFTER = int(_env_float("AXIOM_DAEMON_RECONCILE_ERROR_ESCALATE_AFTER", 3))
# After a transient error, retry on this short cadence instead of the full 600s.
_RECONCILE_ERROR_RETRY_SECONDS = _env_float("AXIOM_DAEMON_RECONCILE_ERROR_RETRY_SECONDS", 45)
...
_RECONCILE_OUTAGE_ESCALATE_SECONDS = _env_float(
    "AXIOM_DAEMON_RECONCILE_OUTAGE_ESCALATE_SECONDS", 900
)
...
RISK_CLOSE_TIMEOUT_SECONDS = _env_float("AXIOM_DAEMON_CLOSE_TIMEOUT_SECONDS", 20)
HEARTBEAT_SEND_TIMEOUT_SECONDS = _env_float("AXIOM_DAEMON_HEARTBEAT_TIMEOUT_SECONDS", 20)
FALLBACK_MIDS_TIMEOUT_SECONDS = _env_float("AXIOM_DAEMON_MIDS_TIMEOUT_SECONDS", 8)
CANDLE_FETCH_TIMEOUT_SECONDS = _env_float("AXIOM_DAEMON_CANDLE_TIMEOUT_SECONDS", 12)
SHUTDOWN_GRACE_SECONDS = _env_float("AXIOM_DAEMON_SHUTDOWN_GRACE_SECONDS", 5)
...
def _sweep_orphan_multiprocessing_workers() -> int:
    ...
    Why: on Windows, taskkill /F or an unclean crash of a Axiom process does not terminate
...
def get_daemon_lock_status() -> dict[str, object]:
    ...
    lock_path = AXIOM_HOME / "daemon.lock"
...
def send_heartbeat(state: dict, prices: dict):
    ...
    sent_msg = ""
    try:
        from axiom.strategies.sentiment import analyze_sentiment
    ...
    msg = (
        f"axiom HEARTBEAT — {datetime.now(timezone.utc).strftime('%H:%M UTC')}\n"
...
def _daemon_startup_bookkeeping(install_signal_handlers: bool) -> dict | None:
    ...
    in-process variant used by `Axiom.api`. Returns the initial state dict
    ...
    log.info("=" * 50)
    log.info("axiom DAEMON v2.0 — DATA/RISK LOOP STARTING")
    log.info("=" * 50)
...
class DaemonLoopDeclined:
    ...
    is already held by another instance (e.g. a standalone ``Axiom daemon
...
async def run_in_loop() -> "DaemonLoopDeclined | None":
    ...
    `Axiom daemon start` process. The file lock still prevents duplicate
