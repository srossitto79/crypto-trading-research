...
// ============== Axiom Classic Compatibility ==============

export interface AxiomDashboardResponse {
...
export interface AxiomRiskStatus {
...
export interface AxiomRegimeSnapshot {
	[asset: string]: {
		regime?: string;
		confidence?: number;
		adx?: number;
...
export interface AxiomEquityPoint {
	time: string;
	value: number;
	pnl: number;
	is_current?: boolean;
...
export interface AxiomEquityHistory {
	base: number;
	curve: AxiomEquityPoint[];
}

export interface AxiomSentimentSnapshot {
	composite?: number;
	[key: string]: unknown;
}

export interface AxiomTrade {
	id?: string;
	asset?: string;
	direction?: string;
	strategy?: string;
...
export interface AxiomTradesPage {
	trades: AxiomTrade[];
	total: number;
	limit: number;
	offset: number;
...
export interface AxiomStrategyPerformance {
	strategy?: string;
	total_trades?: number;
	wins?: number;
	losses?: number;
...
export interface AxiomAgentTask {
	id?: string | number;
	agent_id?: string;
	type?: string;
	title?: string;
...
export interface AxiomSchedulerJob {
	id?: string | number;
	name?: string;
...
export interface AxiomScannerState {
	strategies?: unknown[];
	[key: string]: unknown;
}
...
export interface AxiomAgentDocumentsResponse {
	soul: string;
	agents: string;
	role: string;
}

export interface AxiomAgentUpdatePayload {
	name?: string;
	role?: string;
	model?: string;
	model_id?: string | null;
...
export interface AxiomAgentDocumentBody {
	content: string;
}

export interface AxiomAgentDiscordTestResponse {
	status: string;
	agent_id: string;
	agent_name?: string;
	channel: string;
...
export interface AxiomAgentModelOption {
	key: string;
...
export interface AxiomAgentModelOptionsResponse {
	options?: AxiomAgentModelOption[];
	providers?: Array<{
		provider: string;
...
export interface AxiomAuthProviderStatus {
	provider: AxiomProvider;
	configured: boolean;
	/**
...
export interface AxiomAuthProviderProfilePayload {
	access_token?: string;
	api_key?: string;
...
export interface AxiomAuthProviderOAuthStartResponse {
	provider: AxiomProvider;
...
export type AxiomAuthProviderOAuthStatus =
...
export interface AxiomAuthProviderOAuthCompleteBody {
	code?: string;
	state: string;
	code_verifier?: string;
}

export interface AxiomAuthProviderOAuthCompleteResponse {
	ok: boolean;
	provider: AxiomProvider;
...
export interface AxiomAuthProvidersResponse {
...
export interface AxiomModelPolicyFallbackEntry {
	provider: string;
	model_id: string;
}

export interface AxiomModelPolicyResponse {
	primary_provider: 'minimax' | 'openai' | string;
	primary_model: string;
	provider_priority: string[];
	default_models: Record<string, string>;
	fallback_chains: Record<string, AxiomModelPolicyFallbackEntry[]>;
}
 ...
 */
export interface AxiomModelPolicyUpdatePayload {
...
export interface AxiomAgentTerminalResponse {
	memory?: string;
	logs?: Array<{
		id?: number;
		level?: string;
...
export interface AxiomNotification {
	id: number;
	group_key: string;
	event_type: string;
...
export async function getAxiomStats(): Promise<Record<string, number>> {
	return fetchApi('/axiom/stats');
}

export async function getAxiomDashboard(): Promise<AxiomDashboardResponse> {
	// Canonical endpoint. `/axiom/dashboard` is a deprecated shim that proxies to
...
export async function getAxiomRisk(): Promise<AxiomRiskStatus> {
	// Canonical endpoint. `/axiom/risk` is a deprecated shim that proxies to the
	// same control-plane handler and returns the identical shape.
	return fetchApi('/api/risk');
}

export async function getAxiomRegime(): Promise<AxiomRegimeSnapshot> {
	return fetchApi('/axiom/regime');
}

export async function getAxiomSentiment(): Promise<AxiomSentimentSnapshot> {
	return fetchApi('/axiom/sentiment');
}

export async function getAxiomOpenTrades(): Promise<AxiomTrade[]> {
	return fetchApi('/axiom/trades/open');
}

export async function getAxiomRecentTrades(limit = 20): Promise<AxiomTrade[]> {
	return fetchApi(`/axiom/trades/recent?limit=${limit}`);
}
...
export async function forceCloseAxiomTrade(
	tradeId: string,
	...
	return fetchApi(`/axiom/trades/${encodeURIComponent(tradeId)}/force-close`, {
		method: 'POST',
		body: JSON.stringify({ reason }),
	});
}

export async function getAxiomAllTrades(
	opts: { status?: string; limit?: number; offset?: number } = {}
): Promise<AxiomTradesPage> {
	const params = new URLSearchParams();
...
export async function markAxiomTradeFailed(
	tradeId: string,
...
export async function getAxiomScannerState(): Promise<AxiomScannerState> {
	return fetchApi('/axiom/scanner/state');
}

export async function getAxiomStrategyPerformance(): Promise<AxiomStrategyPerformance[]> {
	return fetchApi('/axiom/strategies/performance');
}

export async function getAxiomAgentTasks(): Promise<AxiomAgentTask[]> {
	return fetchApi('/axiom/agent-tasks');
}

export async function dismissAxiomAgentTask(
...
export async function getAxiomSchedulerJobs(): Promise<AxiomSchedulerJob[]> {
	return fetchApi('/axiom/scheduler');
}
...
export async function updateAxiomSchedulerJob(
	jobId: string | number,
	scheduleType: string,
	scheduleExpr: string,
	enabled?: boolean
...
export async function getAxiomAgents(): Promise<AxiomAgent[]> {
	return fetchApi('/axiom/agents');
}

export async function getAxiomAgentDocuments(agentId: string): Promise<AxiomAgentDocumentsResponse> {
	return fetchApi(`/axiom/agents/${encodeURIComponent(agentId)}/documents`);
}

export async function updateAxiomAgent(agentId: string, payload: AxiomAgentUpdatePayload): Promise<AxiomAgent> {
	return fetchApi(`/axiom/agents/${encodeURIComponent(agentId)}`, {
		method: 'PATCH',
		body: JSON.stringify(payload),
	});
}

export interface AxiomCreateStrategyDeveloperPayload {
	name: string;
	model?: AxiomProvider;
	model_id?: string;
	instructions?: string;
...
export async function createAxiomStrategyDeveloperAgent(
	payload: AxiomCreateStrategyDeveloperPayload
): Promise<AxiomAgent> {
	return fetchApi('/axiom/agents/strategy-developers', {
		method: 'POST',
		body: JSON.stringify(payload),
	});
}

export async function deleteAxiomAgent(agentId: string): Promise<{ ok: boolean; deleted_agent_id: string }> {
	return fetchApi(`/agents/${encodeURIComponent(agentId)}`, {
		method: 'DELETE',
	});
}

export async function updateAxiomAgentDocument(agentId: string, document: 'soul' | 'agents' | 'role', content: string): Promise<{ ok: boolean }> {
	return fetchApi(`/axiom/agents/${encodeURIComponent(agentId)}/documents/${document}`, {
		method: 'PUT',
		body: JSON.stringify({ content }),
	});
}

export async function testAxiomAgentDiscord(
	agentId: string,
	discordToken?: string
): Promise<AxiomAgentDiscordTestResponse> {
	const token = (discordToken ?? '').trim();
	return fetchApi(`/axiom/agents/${encodeURIComponent(agentId)}/test-discord`, {
		method: 'POST',
		body: JSON.stringify(token ? { discord_token: token } : {}),
	});
}

export async function getAxiomAgentModelOptions(refresh = false): Promise<AxiomAgentModelOptionsResponse> {
	const params = refresh ? '?refresh=true' : '';
	return fetchApi(`/axiom/agents/model-options${params}`);
}

export async function getAxiomAuthProviders(): Promise<AxiomAuthProvidersResponse> {
	return fetchApi('/axiom/auth/providers');
}

export async function setAxiomAuthProvider(
	provider: string,
	payload: AxiomAuthProviderProfilePayload
): Promise<{ ok: boolean; provider: string }> {
	return fetchApi(`/axiom/auth/providers/${encodeURIComponent(provider)}`, {
		method: 'POST',
		body: JSON.stringify(payload),
	});
}

export async function startAxiomAuthProviderOAuth(provider: string): Promise<AxiomAuthProviderOAuthStartResponse> {
	return fetchApi(`/axiom/auth/providers/${encodeURIComponent(provider)}/oauth/start`, {
		method: 'POST'
	});
}

export async function completeAxiomAuthProviderOAuth(
	provider: string,
	payload: AxiomAuthProviderOAuthCompleteBody
): Promise<AxiomAuthProviderOAuthCompleteResponse> {
	return fetchApi(`/axiom/auth/providers/${encodeURIComponent(provider)}/oauth/complete`, {
		method: 'POST',
		body: JSON.stringify(payload)
	});
}

export async function pollAxiomAuthProviderOAuth(
	provider: string,
	state: string
): Promise<AxiomAuthProviderOAuthStatus> {
	return fetchApi(
		`/axiom/auth/providers/${encodeURIComponent(provider)}/oauth/status?state=${encodeURIComponent(state)}`
	);
}

export async function cancelAxiomAuthProviderOAuth(
	provider: string,
	state: string
): Promise<{ ok: boolean; provider: string }> {
	return fetchApi(
		`/axiom/auth/providers/${encodeURIComponent(provider)}/oauth/cancel?state=${encodeURIComponent(state)}`,
		{ method: 'POST' }
	);
}

export async function deleteAxiomAuthProvider(provider: string): Promise<{ ok: boolean; provider: string; removed?: boolean }> {
	return fetchApi(`/axiom/auth/providers/${encodeURIComponent(provider)}`, {
		method: 'DELETE',
	});
}

export async function testAxiomAuthProvider(provider: string): Promise<{ ok: boolean; provider: string; status: string; message?: string }> {
	return fetchApi(`/axiom/auth/providers/${encodeURIComponent(provider)}/test`, {
		method: 'POST',
	});
}

export async function getAxiomModelPolicy(): Promise<AxiomModelPolicyResponse> {
	return fetchApi('/axiom/model-policy');
}
 ...
 */
export async function updateAxiomModelPolicy(
	payload: AxiomModelPolicyUpdatePayload
): Promise<AxiomModelPolicyResponse> {
	return fetchApi('/axiom/model-policy', {
		method: 'PUT',
		body: JSON.stringify(payload),
	});
...
export async function updateAxiomAgentModel(
	agentId: string,
	payload: { model: AxiomProvider; model_id?: string }
): Promise<AxiomAgent> {
	return fetchApi(`/axiom/agents/${encodeURIComponent(agentId)}/model`, {
		method: 'PATCH',
		body: JSON.stringify(payload),
	});
}

export async function getAxiomBacktestingStatus(): Promise<AxiomBridgeStatus> {
	return fetchApi('/axiom/backtesting/status');
}
...
export async function acknowledgeNotification(notificationId: number): Promise<{ ok: boolean; item: AxiomNotification }> {
...
export async function resendNotification(notificationId: number): Promise<{ ok: boolean; item: AxiomNotification }> {
...
): Promise<{ ok: boolean; item: AxiomNotification }> {
...
export async function setAxiomExecutionMode(mode: 'paper' | 'live' | 'mainnet'): Promise<{ ok: boolean; mode: string; previous: string }> {
	return fetchApi('/axiom/execution-mode', {
		method: 'POST',
		body: JSON.stringify({ mode, confirm: true }),
	});
}

export async function triggerAxiomEmergencyHalt(): Promise<{ ok: boolean; closed?: unknown[] }> {
	return fetchApi('/axiom/emergency-halt', {
		method: 'POST',
		body: JSON.stringify({ confirm: true }),
	});
}

export async function resetAxiomKillSwitch(): Promise<{ ok: boolean }> {
	return fetchApi('/axiom/kill-switch/reset', {
		method: 'POST',
		body: JSON.stringify({ confirm: true }),
	});
}

export async function toggleAxiomKillSwitch(enabled: boolean): Promise<{ ok: boolean; kill_switch_enabled: boolean }> {
	return fetchApi('/axiom/kill-switch/toggle', {
		method: 'POST',
...
function resolveWsApiKey(): string {
	...
	try {
		const fromEnv = String((import.meta as { env?: Record<string, unknown> }).env?.VITE_AXIOM_API_KEY ?? '').trim();
	...
	try {
		if (typeof window !== 'undefined') {
			return (window.localStorage.getItem('axiom_api_key') ?? '').trim();
		}
	} catch {
		/* ignore */
	}
...
export function getAxiomLiveWebSocketUrls(): string[] {
...
export function getAxiomLiveWebSocketUrl(): string {
	const urls = getAxiomLiveWebSocketUrls();
	return urls[0] ?? toLiveWsUrl(ACTIVE_API_BASE || API_BASE || '/api');
}

export async function getAxiomEquityHistory(): Promise<AxiomEquityHistory> {
	return fetchApi('/axiom/equity-history');
}
...
export async function getAxiomLogs(limit = 100): Promise<AxiomLogEntry[]> {
	return fetchApi(`/axiom/logs?limit=${limit}`);
}

export async function getAxiomAgentTerminal(agentId: string): Promise<AxiomAgentTerminalResponse> {
	return fetchApi(`/agents/${encodeURIComponent(agentId)}/terminal`);
}

export async function getAxiomEvolution(): Promise<Record<string, number>> {
	return fetchApi('/axiom/evolution');
}

export async function getAxiomStrategies(status?: string): Promise<Array<Record<string, unknown>>> {
	const params = status ? `?status=${encodeURIComponent(status)}` : '';
	return fetchApi(`/axiom/strategies${params}`);
}

export interface AxiomStrategyQuery {
	status?: string;
	owner?: string;
	limit?: number;
	offset?: number;
...
export async function getAxiomStrategiesQuery(query: AxiomStrategyQuery = {}): Promise<Array<Record<string, unknown>>> {
	const params = new URLSearchParams();
	...
	return fetchApi(`/axiom/strategies${queryString ? `?${queryString}` : ''}`);
}
...
export async function promoteAxiomStrategy(
	strategyId: string,
	toStatus: string,
	...
	const result = await fetchApi<{
		...
		to_status?: string;
		updated_at?: string;
	}>(`/axiom/strategies/${encodeURIComponent(strategyId)}/promote`, {
		method: 'POST',
		body: JSON.stringify({
			to_status: toStatus,
...
export async function getSettings(): Promise<AxiomSettings> {
...
export interface SystemHeartbeatResponse {
	dashboard: AxiomDashboardResponse;
	risk: AxiomRiskStatus;
	sentiment: AxiomSentimentSnapshot;
	regime: AxiomRegimeSnapshot;
	scanner_state: AxiomScannerState;
	open_trades: AxiomTrade[];
