"""Complete gauntlet for S03096 repurposed as engulfing ETH/USDT 4h."""
...
if __name__ == '__main__':
    ...
    ASSET = 'ETH/USDT'
    STYPE = 'engulfing'
    PARAMS = {'volume_mult': 2.0, 'atr_period': 14}
    TF = '4h'

    conn = sqlite3.connect(AXIOM_DB)
    conn.row_factory = sqlite3.Row

    # Update the strategy record to use engulfing ETH/USDT 4h
    conn.execute("""
        UPDATE strategies SET type=?, symbol=?, timeframe=?, params=?, status='gauntlet', stage='gauntlet'
        WHERE id=?
    """, (STYPE, ASSET, TF, json.dumps(PARAMS), SID))
    conn.execute("DELETE FROM gate_rejections WHERE strategy_id=?", (SID,))
    conn.execute("DELETE FROM backtest_results WHERE strategy_id=?", (SID,))
    conn.commit()
    print(f"Updated {SID} to engulfing ETH/USDT 4h and restored to gauntlet")
    ...
    if oos_t == 0:
        print("ERROR: 0 OOS trades — aborting")
        sys.exit(1)
