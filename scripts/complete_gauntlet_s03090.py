"""Complete gauntlet run for S03090 (zscore_reversion ETH/USDT 1h, no adx_max override)."""
...
if __name__ == '__main__':
    ...
    SID = 'S03090'
    ASSET = 'ETH/USDT'
    STYPE = 'zscore_reversion'
    # No adx_max override — use implicit regime filter (ADX<25)
    PARAMS = {'period': 20, 'entry_threshold': 1.8, 'exit_threshold': 0.3}
    TF = '1h'
    ...
    # Run backtest
    print(f"Running backtest for {SID}... (params={PARAMS})")
    bt = backtest_strategy(
        strategy_id=SID, asset=ASSET, strategy_type=STYPE, params=PARAMS,
        bars=17520, timeframe=TF, persist_legacy_run=True, regime_gate=True,
    )
    if 'error' in bt:
